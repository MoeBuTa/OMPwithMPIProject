...
/**
 * @brief  measure the time elasped by omp+mpi solution and verify the result
 * @note   
 ...
 * @retval 
 */
void *ompmpiSortMeasure(char *algo, int subArrLen, int subBufLen, int length, int rank, int size, int threads, MPI_Status status)
{
    double *subArr = mpiRead("mpi_unsorted_array.bin", subArrLen, subBufLen, rank, size, status);
    if (rank == 0)
    {
        printf("Computing OMP+MPI %s sort...\n", algo);
        gettimeofday(&start, NULL);
    }

    selectParallelAlgorithm(algo, subArr, subArrLen, threads);
    ...
    if (rank == 0)
    {
        ...
        printf("time spent by OMP+MPI %s sort: %lfs\n", algo, delta);

        //write result into csv file
        write_csv(length, algo, size, threads, delta);
